...
export default function Knives(){
  ...
  useEffect(()=>{
    ...
    function throwKnife(){
      ...
      for(const a of knives){
        const d=Math.abs(a-kAngle);
        if(d<0.2||Math.abs(d-2*Math.PI)<0.2){
          // Game over - check if new highscore
          if (h > best) {
            setBest(h)
            saveHighscore(gameId, h)
          }
          h=0;
          knives=[];
          speed=0.03;
      ...
      h++;
      setHits(h);
      if(knives.length%6===0) speed+=0.01
    }
    ...
    const loop=()=>{
      ...
      knives.forEach(a=>{
        ...
        x.lineTo(cx+Math.cos(a)*(r+18),cy+Math.sin(a)*(r+18));
        x.stroke()
      })
      x.strokeStyle='#f1c40f';
      x.beginPath();
      x.moveTo(cx+Math.cos(angle)*(r-10),cy+Math.sin(angle)*(r-10));
      x.lineTo(cx+Math.cos(angle)*(r+10),cy+Math.sin(angle)*(r+10));
      x.stroke()
  ...
  return (<div>
    <div className="controls">
      <span className="pill">Steuerung: <b>Space/Klick</b> Messer werfen</span>
      <span className="pill">Treffer: <span className="score">{hits}</span></span>
